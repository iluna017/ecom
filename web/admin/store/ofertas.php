<html>
	...
	<body>
		...
		<div class="container">
			...
			<div class="backtable center" id="tabla-productosTable" name="tabla-productosTable">
			...
			<div class="backtable center">
				<table class="table table-hover" id="tabla-ofertas"
				name="tabla-ofertas">
					<thead style="background-color: #153CFF">
						<tr style="color: #fff">
							<!--th>No</th-->
							<!--th></th-->
							<th width="80%">En Oferta</th>
							<th></th>
							<th></th>
							<th></th>
						</tr>
...
<script type="text/javascript">
	...
	function loadOfertas() {
		var params = {
			operations : [{
				...
				in_1 : 'Select IdProducto from ofertas'
			}]
		};
		...
		if (data.length > 0) {
			...
			var actionSelect = "SELECT_IdCategoria,IdProducto_<img  width='30' height='30' src='../images/minus	.png' onclick='removerOfertasBtn(this,IdCategoria,IdProducto)'>";
			var colNames = ["Nombre", actionSee, actionEdit, actionSelect];
			fillTable('tabla-ofertas', colNames, data, '190px');
		}
	}
	...
	function loadProductos() {
		var params = {
			operations : [{
				...
				tableName : 'productos',
				columnNames : 'IdCategoria,IdProducto,Nombre',
				where : 'IdCategoria=' + $('#cmbCategorias').val() + ' AND IdProducto NOT IN (Select IdProducto from ofertas)'
			}]
		};
		var myJSONText = JSON.stringify(params);
		var data = execAjax(myJSONText, "POST", "json", "../../phpControllers/dbController.php");
		var actionAdd = "SELECT_IdCategoria,IdProducto_<img  width='30' height='30' src='../images/add.png' onclick='agregarOfertaBtn(this,IdCategoria,IdProducto)'>";
		var actionEdit = "ACTION_IdCategoria,IdProducto_<a href='#' onclick='editarProductoBtn(this,IdCategoria,IdProducto)'><img  width='30' height='30' src='../images/edit.png'></a>";
		var actionSee = "ACTION_IdCategoria,IdProducto_<a href='#' onclick='verProductoBtn(IdCategoria,IdProducto)'><img  width='30' height='30' src='../images/see2.png'></a>";
	...
	function agregarOfertaBtn(ref, IdCategoria, IdProducto) {
		waitingDialog.show('Agregando Oferta...');
		setTimeout(function() {
			agregaOferta(ref, IdCategoria, IdProducto);
		}, 1000);
	}

	function agregaOferta(ref, IdCategoria, IdProducto) {
		var params = {
			operations : [{
				operation : 'insertar',
				tableName : 'ofertas',
		...
		if (data["ESTATUS"] == 0) {
			$('#tabla-ofertas').append('<tr id="' + trId + '">' + $('tr[id="' + trId + '"]').html().replace('add', 'minus').replace('agregarOfertaBtn', 'removerOfertasBtn') + '</tr>');
	...
	function removerOfertasBtn(ref, IdCategoria, IdProducto) {
		waitingDialog.show('Eliminando Oferta...');
		setTimeout(function() {
			removerOfertas(ref, IdCategoria, IdProducto);
		}, 1000);
	}

	function removerOfertas(ref, IdCategoria, IdProducto) {
		var params = {
			operations : [{
				operation : 'delete',
				tableName : 'ofertas',
		...
		if (data["MSG"].indexOf('Error') == -1) {
			//$('#tabla-productos').append('<tr id="'+trId+'">'+$('tr[id="' + trId + '"]').html().replace('add','minus')+'</tr>');
			$(ref).closest('tr').remove();
		}
	}
</script>
