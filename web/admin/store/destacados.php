...
<script type="text/javascript">
	...
	function loadDestacados() {
		var params = {
			operations : [{
				...
				in_1 : 'Select IdProducto from destacados'
			}]
		};
	...
	function agregarDestacadosBtn(ref, IdCategoria, IdProducto) {
		waitingDialog.show('Agregando Destacado...');
		setTimeout(function() {
	...
	function agregaDestacados(ref, IdCategoria, IdProducto) {
		...
		if (data["ESTATUS"] == 0) {
			$('#tabla-destacados').append('<tr id="' + trId + '">' + $('tr[id="' + trId + '"]').html().replace('add', 'minus') + '</tr>');
	...
	function removerDestacadosBtn(ref, IdCategoria, IdProducto) {
		waitingDialog.show('Eliminando Destacado...');
		setTimeout(function() {
